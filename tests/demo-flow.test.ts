...
describe("demo flow persistence", () => {
  ...
  it("generates the initial Sabine strategy and phone-call action", () => {
    const detail = db.generateStrategy(seed.DEMO_QUOTE_ID);

    expect(detail.strategy?.headline).toContain("Win Sabine");
    expect(detail.quote.nextAction.kind).toBe("schedule_call");
    expect(detail.actions.some((action) => action.id === "action_sabine_call")).toBe(true);
  });

  it("schedules the call and creates a highlighted calendar event", () => {
    const generated = db.generateStrategy(seed.DEMO_QUOTE_ID);
  ...
  it("uses the call debrief to recommend an in-person visit", () => {
    const generated = db.generateStrategy(seed.DEMO_QUOTE_ID);
    const scheduled = db.scheduleAction(generated.quote.nextAction.actionId!, {});
    const updated = db.logAction(scheduled.quote.nextAction.actionId!, {
      notes:
    ...
    expect(updated.strategy?.steps.find((step) => step.id === "step_visit")?.status).toBe("active");
  });

  it("uses the visit debrief to recommend a final recap and can close the quote", () => {
    const generated = db.generateStrategy(seed.DEMO_QUOTE_ID);
    const callScheduled = db.scheduleAction(generated.quote.nextAction.actionId!, {});
    const visitRecommended = db.logAction(callScheduled.quote.nextAction.actionId!, {
      notes:
        "Customer said the price may be okay, but they want someone to come look at the house, roof, and cable path before they trust the final quote.",
    });
    const visitScheduled = db.scheduleAction(visitRecommended.quote.nextAction.actionId!, {});
    const finalRecommended = db.logAction(visitScheduled.quote.nextAction.actionId!, {
      notes:
  ...
  it("marks an existing strategy stale after master data edits", () => {
    db.generateStrategy(seed.DEMO_QUOTE_ID);
